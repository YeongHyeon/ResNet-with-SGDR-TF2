...
class CNN(object):
    ...
    def __init__(self, height, width, channel, num_class, ksize, learning_rate=1e-3, ckpt_dir='./Checkpoint'):
        ...
        print("\nInitializing Short-ResNet with SGDR...")
        ...
        self.lr_max, self.lr_min, self.lr_tmp = self.learning_rate, self.learning_rate / 5, 0
        self.T_i, self.T_mult, self.T_cur = 10, 1, 0
    ...
    def step(self, x, y, epoch=0, iteration=0, iter_per_epoch=0, train=False):
        ...
        if(train):
            ...
            self.lr_tmp = self.lr_min + \
                0.5 * (self.lr_max - self.lr_min) * (1 + np.cos(self.T_cur / self.T_i * np.pi))
            self.T_cur = int(self.T_cur) + (((iteration % iter_per_epoch)+1) / iter_per_epoch)

            optimizer = tf.optimizers.SGD(learning_rate=self.lr_tmp, nesterov=True)
            optimizer.apply_gradients(zip(gradients, self.customlayers.params_trainable))

            with self.summary_writer.as_default():
                tf.summary.scalar('ResNet/loss', loss, step=iteration)
                tf.summary.scalar('ResNet/accuracy', accuracy, step=iteration)
                tf.summary.scalar('ResNet/learning_rate', self.lr_tmp, step=iteration)
